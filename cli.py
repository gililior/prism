...
def main():
    ...
    ap.add_argument("--paper", type=str, help="Path to JSON paper (parsed).")
    ap.add_argument("--pdf", type=str, help="Path to paper PDF. If provided, overrides --paper.")
    ...
    if args.pdf:
        paper = parse_pdf_file_to_paper(args.pdf)
    else:
        paper_json = json.load(open(args.paper, "r", encoding="utf-8"))
        paper = parse_pdf_to_paper(paper_json)
    ...
    # Dynamic routing: select facets and sections
    router = DynamicRouter()
    if args.routing == "dynamic":
        routed = router.route(paper)
    else:
        # Build routed dict for all facets using accumulated span texts; fallback to full paper
        facet_texts = {f: [] for f in cfg.facets}
        for sec in paper.sections:
            for sp in getattr(sec, "spans", []) or []:
                for f in sp.facets:
                    if f in facet_texts:
                        facet_texts[f].append(sp.text)
        routed = {}
        full_text = "\n\n".join([s.text for s in paper.sections])
        for f in cfg.facets:
            txts = facet_texts.get(f) or []
            routed[f] = {"sections": [s.name for s in paper.sections], "text": ("\n\n".join(txts) if txts else full_text)}

    # Run selected reviewers only
    all_points = []
